...
public:
    ...
    struct conv_opt
    {
        conv_opt(int in, int out, int kernel)
            : in_channels(in), out_channels(out), kernel_size(kernel)
        {}
        int in_channels;
        int out_channels;
        int kernel_size;
...
void load_net(std::string Path, Net_archictec& Model, neural_options& options_)
{
    if (std::filesystem::exists(Path))
    {
        auto load_model_ptr = std::make_shared<Net_archictec>(options_);
