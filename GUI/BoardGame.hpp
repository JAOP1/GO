...
private:
    ...
    bool is_ko(vertex current_piece) const;
...
std::vector<int> BoardGame::make_action(vertex v)
{
    ...
    // Esto es para revisar que no se genere un ko.
    last_piece_dead = -1;
    if (result.size() == 1)
        last_piece_dead = result[0];
    last_piece = v;
    return result;
}
...
bool BoardGame::is_valid_move(vertex v) const
{
    ...
    // Aun tengo que revisar cosas del ko....
    if ((!empty_cells.empty() || min_liberties == 0) && !is_ko(v))
        ...
        if (v == root)
        {
            ...
            for (auto neighbor : Board.neighbors(node_))
            {
                    ...
                    pieces[neighbor].type != 'N')
                {
                    ...
                    if (pieces[current].libertiesNodes.find(node_) ==
                        pieces[current].libertiesNodes.end())
                        pieces[current].libertiesNodes.insert(node_);
                }
            }

            groups.disjoint_element(node_);
            pieces[node_].reset();
...
std::vector<int> BoardGame::dead_vertices(vertex node)
{
    ...
    {
        // Revisa grupos de piezas del adversario,
        // si ya no tiene grados de libertad, muere el grupo.
...
bool BoardGame::is_ko(vertex current_piece) const
{
    if (last_piece_dead != -1 && last_piece != -1)
    {
        if (Board.is_neighbor(current_piece, last_piece))
        {

            if (liberties(last_piece) == 1)
            {
                if (last_piece_dead == current_piece)
                    return true;
            }
        }
    }
