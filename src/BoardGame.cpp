...
void BoardGame::debug_function() const
{
    // Grupos con vida.
...
int BoardGame::reward(char player) const
{
    ...
    // La región que domina el jugador y las piezas capturadas del adversario es
    // la recompensa.

    if (player == 'B')
        return (dominated_by_white + points_by_white <
                    dominated_by_black + points_by_black
                  ? 1
                  : -1);

    return (dominated_by_black + points_by_black <
                dominated_by_white + points_by_white
              ? 1
              : -1);
}
...
void BoardGame::find_alive_groups(std::vector<char>& pieces_color) const
{
    ...
    for (auto v : Board.vertices())
    {
        ...
        if (!pieces[label_].is_alive)
        {
            ...
            for (it;
                 it != pieces[label_].libertiesNodes.end() && eyes_counter < 2;
                 ++it)
            {
                vertex cell = *it;
